    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'Add Patient' in request.POST:
            ...
            if AddPatientform.is_valid():
                ...
                # Redirect to a GET request for the same view.
                ...
                if(('Patient Outcome-%s' % p.pk) in request.POST ):
                    ...
                    if formset.is_valid():
                        for form in formset.forms:
                            identical_POs = PatientOutcome.objects.filter(
                                patient=p, stepOutcome=form.cleaned_data['stepOutcome'])
                            if identical_POs.exists() :
                                identical_POs.delete()
...
# Can remove these detail and thanks views, and the corresponding urlconfs.
# They won't really be used in the real app.
